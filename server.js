...
/* Convert different possible frontend values into a real boolean. */
...
const thinkHarder = isEnabled(req.body.thinkHarder);

/*
  IMPORTANT:
  Web search is controlled ONLY by the searchWeb toggle.
  If it is false/off, searchWeb() is NEVER called.
*/
const searchWebEnabled = isEnabled(req.body.searchWeb);
...
if (searchWebEnabled && userQuery.trim()) {
  ...
  console.log("WEB SEARCH TOGGLE: OFF - NO SEARCH");
}
